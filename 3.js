...
Question 3: changePossibilities
  ...
  your program would output 4—the
...
/* changePossibilities uses dynammic programming to solve the problem.
  ...
  This function creates a new array (combinations) whose length corresponds to the value that needs to be converted
  into change. ChangePossibilities then uses a nested loop to check if each coin's
  value is less than or equal to the array's value at the current index. (i.e. you can't use $5 to make change for $1).
  If true, the function takes the combination at the index of the current value less the coin value
  and adds it the total possibilites.
